...
func concatStringByStringBuilder(sl []string) string {
...
func concatStringByStringBuilderWithInitSize(sl []string) string {
	var b strings.Builder
	b.Grow(64)
	for _, v := range sl {
		b.WriteString(v)
...
func BenchmarkConcatStringByStringsBuilderWithInitSize(b *testing.B) {
	for n := 0; n < b.N; n++ {
		concatStringByStringBuilderWithInitSize(sl)
	}
}
